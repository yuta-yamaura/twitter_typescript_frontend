...
export const UserBookmarkList = () => {
  ...
  const fetchUserBookmarkList = async () => {
    try {
      const res = await authInstance.get<PaginatedResponse<Bookmark>>(`/api/bookmark/`);
      setUserBookmark(res.data.results);
    } catch (error) {
                      ...
                      >
                        <div>
                          ...
                          <span>
                            {" "}
                            {bookmark?.createdAt &&
                              dayjs(bookmark.createdAt).format("YYYY年M月D日")}
                          </span>
                        </div>
                      </div>
                      <div>
                        <Flex>{bookmark.content}</Flex>
                        {bookmark.image && (
                          <Flex
                            style={{
                              alignItems: "center",
